...
public class LTC_Worker extends Observable implements Runnable {
	...
	private volatile LTC_Work curWork = null;
	private URL lpUrl = null;
	private HttpURLConnection lpConn = null;
	private AtomicLong hashes = new AtomicLong(0L);

	public LTC_Worker(URL url, String auth, long scanMillis, long pauseMillis) {
		this(url, auth, scanMillis, pauseMillis, Runtime.getRuntime().availableProcessors());
	}

	public LTC_Worker(URL url, String auth, long scanMillis, long pauseMillis, int nThreads) {
		this(url, auth, scanMillis, pauseMillis, nThreads, 1.0);
	}

	public LTC_Worker(URL url, String auth, long scanMillis, long pauseMillis, int nThreads, double throttle) {
		this.url = url;
		this.auth = auth;
	...
	private synchronized LTC_Work getWork() {
		while (running) {
			try {
				return new LTC_Work(url, auth);
			} catch (Exception e) {
				if (!running)
					break;
        		setChanged();
		...
		public void run() {
			while (running) {
				try {
					...
					curWork = new LTC_Work(lpConn, url, auth);
					if (!running)
						break;
	        		synchronized (LTC_Worker.this) {
			    		setChanged();
	...
	private class WorkChecker implements Runnable {
		...
		public void run() {
			try {
				...
				LTC_Hasher hasher = new LTC_Hasher();
	...
	private class WorkSubmitter implements Runnable {
		private LTC_Work work;
		private int nonce;
		public WorkSubmitter(LTC_Work w, int nonce) {
			this.work = w;
			this.nonce = nonce;
		}
		public void run() {
